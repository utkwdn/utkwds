...
import { parseIcon as parseIconCurrent } from './../utils/parse-icon';
...
// v4 matches the save output from before the icon `width` stopped being
// serialized as an inline style (and defaulted to "125px"). The `migrate`
// drops that legacy default so existing icons pick up the CSS-driven default
// size and re-save without the inline width style.
const v4 = {
	attributes: {
		icon: {
			type: 'string',
	...
	save( { attributes } ) {
		...
		if ( icon && isEmpty( namedIcon ) ) {
			// Custom icons are strings and need to be parsed.
			printedIcon = parseIconCurrent( icon );
			...
			// Icons provided by third-parties are generally strings.
			if ( typeof printedIcon === 'string' ) {
				printedIcon = parseIconCurrent( printedIcon );
			}
		}
	...
	migrate( attributes ) {
		// "125px" was the old serialized default; drop it so the icon falls
		// back to the CSS default size and re-saves without an inline width.
		if ( attributes.width === '125px' ) {
			const { width, ...rest } = attributes; // eslint-disable-line no-unused-vars
			return rest;
		}
