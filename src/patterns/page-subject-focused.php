<?php
/**
 * Title: Subject Focused Page
 * Slug: utkwds/page-subject-focused
 * Categories: page-layouts
 */
?>

<!-- wp:pattern {"slug":"utkwds/media-text-cta-white"} /-->

<!-- wp:pattern {"slug":"utkwds/text-contact-smokey"} /-->

<!-- wp:pattern {"slug":"utkwds/horizontal-single-white"} /-->

<!-- wp:pattern {"slug":"utkwds/contact-2up-light-gray"} /-->

<!-- wp:pattern {"slug":"utkwds/subject-3up-light-gray"} /-->

<!-- wp:pattern {"slug":"utkwds/cta-link-group-white"} /-->

<!-- wp:pattern {"slug":"utkwds/quote-media-orange-light-gray"} /-->

<!-- wp:pattern {"slug":"utkwds/accordion-white"} /-->

<!-- wp:pattern {"slug":"utkwds/callout-orange"} /-->

<!-- wp:pattern {"slug":"utkwds/text-media-bg-texture-b"} /-->
