...
// ---------------------------------------------------------------------------------
define( 'WDS_CUSTOM_ROLES_VERSION', '1.2.1' );
...
function wds_create_custom_roles() {
	foreach ( wds_get_role_definitions() as $slug => $def ) {
		...
		foreach ( ( $def['add'] ?? array() ) as $cap => $value ) {
			$capabilities[ $cap ] = true;
		}

		// Remove any specified caps.
		foreach ( ( $def['remove'] ?? array() ) as $cap => $value ) {
...
function wds_restrict_admin_bar( WP_Admin_Bar $wp_admin_bar ) {
	...
	$restricted_roles = array( 'wds_administrator', 'wds_editor' );
