...
/*!*******************************************!*\
  !*** ./src/blocks/accordion/accordion.ts ***!
  \*******************************************/
const setUpAccordions = () => {
  const accordions = document.querySelectorAll('[data-accordion]');
  console.log(`Found ${accordions.length} accordions`);
  if (!accordions.length) return;
  const isHTMLSectionElement = el => el.tagName === 'SECTION';
  ...
  const CLASS_NAME_IS_OPEN_POINTER_EVENTS_AUTO = /* tw */'[&.is-open]:pointer-events-auto';
  class Panel {
    constructor(button, section) {
       ...
       */
      section.addEventListener('transitioncancel', ({
        target,
        currentTarget
      }) => {
        if (target !== currentTarget) return;
        section.removeEventListener('transitionend', this._closeTransitionEnd);
    ...
    _openTransitionEnd = ({
      ...
      if (target !== currentTarget) return;
      this.section.removeEventListener('transitionend', this._openTransitionEnd);
      this._toOpenState();
    };
    ...
    _closeTransitionEnd = ({
      ...
      if (target !== currentTarget) return;
      this.section.removeEventListener('transitionend', this._closeTransitionEnd);
      this._toClosedState();
    };
    ...
    close() {
      ...
      if (this.section.contains(document.activeElement) && (document.activeElement instanceof HTMLElement || document.activeElement instanceof SVGElement)) {
      ...
      requestAnimationFrame(() => {
        this.section.addEventListener('transitionend', this._closeTransitionEnd);
  ...
  for (const accordion of accordions) {
    ...
    for (const el of accordion.children) {
      ...
      if (!(heading instanceof HTMLHeadingElement)) {
        console.error('A `utk-wds-accordion__heading` element is missing or is not an h-tag.');
        continue;
      }
      if (!(section instanceof HTMLElement) || !isHTMLSectionElement(section)) {
        console.error('A `[data-accordion-section]` element is missing or is not a `<section>`.');
        continue;
      }
      ...
      section.classList.add( /* tw */'aria-hidden:hidden', /* tw */'overflow-hidden', /* tw */'pointer-events-none', CLASS_NAME_IS_OPEN_POINTER_EVENTS_AUTO);
    ...
    panels.forEach(panel => {
      panel.button.addEventListener('click', closeOthersOnOpen ? () => {
        if (panels.some(({
          isTransitioning
