...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const paginationContainer = document.getElementById('utk-search-pagination');

  /**
   * Fetch results from Google CSE API.
   */
  function fetchResults(startIndex = 1) {
    const url = `${UTK_CSE.endpoint}?key=${UTK_CSE.apiKey}&cx=${UTK_CSE.cx}&q=${encodeURIComponent(query)}&start=${startIndex}`;
    resultsContainer.innerHTML = `<p>Loading…</p>`;
    paginationContainer.innerHTML = '';
    fetch(url).then(res => res.json()).then(renderResults).catch(err => {
...
/******/ })()
;
//# sourceMappingURL=google-cse.js.map